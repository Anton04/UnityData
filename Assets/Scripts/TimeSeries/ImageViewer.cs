using System.Collections;
using System.Collections.Generic;
using UnityEngine;



public class ImageViewer : DataNode
{
    public Texture2D tex;

    bool changed = false;
    public byte[] imgBytes,test1,test2,test3;



    // Start is called before the first frame update
    new void Start()
    {
        base.Start();

        tex = new Texture2D(2, 2);



        // A small 64x64 Unity logo encoded into a PNG.
        var pngBytes = new byte[] {
            ,
        };
        // Load data into the texture.
        tex.LoadImage(pngBytes);
        // Assign texture to renderer's material.
        GetComponent<Renderer>().material.mainTexture = tex;
    }


    public void Show(byte[] pngBytes)
    {

        print(pngBytes);
        imgBytes = pngBytes;
        changed = true;


    }

    // Update is called once per frame
    void Update()
    {
        if (changed)
            tex.LoadImage(imgBytes);

        // Assign texture to renderer's material.
        //GetComponent<Renderer>().material.mainTexture = tex;
    }

    public override void TimeDataUpdate(Subscription Sub, DataPoint data)
    {
        //print("Updating!!!");

        base.TimeDataUpdate(Sub, data);

        //string String, byteArray = "�PNG\\r\\n\\u001a\\n\\u0000\\u0000\\u0000\\rIHDR\\u0000\\u0000";



        //var result = String.Join("", byteArray.Select(b => b > '0' && b < '9' ?
        //((char)b.ToString()) : String.Format(@"\u{0:x4}", b)));

        //print result;

        //test1 = new byte[10];
        test1 = System.Text.Encoding.ASCII.GetBytes(data.Texts[0]);




        //print(data.Texts[0].Length);
        print (System.Convert.ToByte(data.Texts[0][0]));

        //Show( data.Texts[0] );    


    }

}
